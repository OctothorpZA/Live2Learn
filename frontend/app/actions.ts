// /frontend/app/actions.ts

'use server'

import { draftMode } from 'next/headers'
import * as brevo from '@getbrevo/brevo'
// Import the crypto module for generating the PayFast signature
import crypto from 'crypto'
// Import the CartItem type from our Zustand store
// import { CartItem } from './store/cart'
// Import the canonical order from our new utility file
import { PAYFAST_CHECKOUT_PARAM_ORDER } from '@/app/lib/payfast'

// --- Helper function for PayFast URL encoding ---
const pfUrlEncode = (value: string): string => {
  let encodedValue = encodeURIComponent(value.toString().trim()).replace(
    /%20/g,
    '+'
  )
  // Convert all URL-encoded hex codes to uppercase as required by PayFast
  encodedValue = encodedValue.replace(/%[0-9a-f]{2}/g, (match) =>
    match.toUpperCase()
  )
  return encodedValue
}

// --- Define the type for the data returned to the client ---
export type PayFastFormData = {
  actionUrl: string
  inputs: { name: string; value: string }[]
}

// --- Define the type for the checkout payload from the client ---
type CheckoutPayload = {
  orderId: string
  amount: number
  itemName: string
  itemDescription: string
}

// --- PayFast Checkout Server Action ---
export async function processCheckout(
  payload: CheckoutPayload
): Promise<PayFastFormData> {
  'use server'

  const merchantId = process.env.PAYFAST_MERCHANT_ID
  const merchantKey = process.env.PAYFAST_MERCHANT_KEY
  const passphrase = process.env.PAYFAST_PASSPHRASE
  // Use Vercel's system environment variable for the public URL when deployed.
  const siteUrl = process.env.VERCEL_URL
    ? `https://${process.env.VERCEL_URL}`
    : process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'
  const payfastUrl =
    process.env.PAYFAST_PROCESS_URL ||
    'https://sandbox.payfast.co.za/eng/process'

  if (!merchantId || !merchantKey || !passphrase) {
    throw new Error('PayFast credentials are not configured.')
  }

  // 1. Assemble all payment data
  const paymentData: { [key: string]: string } = {
    merchant_id: merchantId,
    merchant_key: merchantKey,
    return_url: `${siteUrl}/payment/success`,
    cancel_url: `${siteUrl}/payment/cancel`,
    notify_url: `${siteUrl}/api/notify`,
    m_payment_id: payload.orderId,
    amount: payload.amount.toFixed(2),
    item_name: payload.itemName,
    item_description: payload.itemDescription,
  }

  // 2. Filter out any blank values
  const filteredData = Object.fromEntries(
    Object.entries(paymentData).filter(
      ([, value]) => value !== '' && value !== null && value !== undefined
    )
  )

  // 3. Build the signature string IN THE CANONICAL ORDER
  let signatureString = ''
  PAYFAST_CHECKOUT_PARAM_ORDER.forEach((key) => {
    if (filteredData[key]) {
      signatureString += `${key}=${pfUrlEncode(filteredData[key])}&`
    }
  })

  // 4. Trim trailing "&" and append passphrase
  signatureString = signatureString.slice(0, -1)
  signatureString += `&passphrase=${pfUrlEncode(passphrase)}`

  // 5. Compute the MD5 hash
  const signature = crypto.createHash('md5').update(signatureString).digest('hex')

  // 6. Add the signature to the data to be sent to PayFast
  const finalData = {
    ...paymentData,
    signature,
  }

  // 7. Prepare the data for the client-side form
  const inputs = Object.entries(finalData).map(([name, value]) => ({
    name,
    value,
  }))

  return {
    actionUrl: payfastUrl,
    inputs,
  }
}

// (Keep your existing Brevo and Draft Mode actions below this line)
// --- Brevo Contact Form Action ---
interface ContactFormData {
  name: string
  email: string
  subject: string
  message: string
}

export async function sendContactEmail(formData: ContactFormData) {
  'use server'

  const { name, email, subject, message } = formData

  // --- 1. Validate the data ---
  if (!name || !email || !subject || !message) {
    return { error: 'All fields are required.' }
  }

  // --- 2. Initialize the Brevo API ---
  const api = new brevo.TransactionalEmailsApi()
  api.setApiKey(
    brevo.TransactionalEmailsApiApiKeys.apiKey,
    process.env.BREVO_API_KEY as string
  )

  // --- 3. Prepare the email ---
  const sendSmtpEmail = new brevo.SendSmtpEmail()
  sendSmtpEmail.subject = `New Contact Form Submission: ${subject}`
  sendSmtpEmail.htmlContent = `
    <html>
      <body>
        <h1>New Message from LTL Website Contact Form</h1>
        <p><strong>Name:</strong> ${name}</p>
        <p><strong>Email:</strong> ${email}</p>
        <p><strong>Subject:</strong> ${subject}</p>
        <p><strong>Message:</strong></p>
        <p>${message.replace(/\n/g, '<br>')}</p>
      </body>
    </html>
  `
  // FIX: Use environment variables for recipient and sender details
  sendSmtpEmail.to = [
    {
      email: process.env.BREVO_TO_EMAIL || 'info@livingthroughlearning.org',
      name: process.env.BREVO_TO_NAME || 'LTL Admin',
    },
  ]
  sendSmtpEmail.sender = {
    email: process.env.BREVO_SENDER_EMAIL || 'noreply@yourdomain.com',
    name: process.env.BREVO_SENDER_NAME || 'LTL Website Form',
  }

  try {
    await api.sendTransacEmail(sendSmtpEmail)
    return { success: true, message: 'Thank you for your message!' }
  } catch (error) {
    console.error('Brevo API Error:', error)
    return {
      error: 'There was an error sending your message. Please try again later.',
    }
  }
}

// --- Draft Mode Action ---
export async function disableDraftMode() {
  'use server'
  await Promise.allSettled([
    (await draftMode()).disable(),
    new Promise((resolve) => setTimeout(resolve, 1000)),
  ])
}
